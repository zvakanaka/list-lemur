...
const bodySnatcher = require('./bodySnatcher.js');
const sendMail = require('./sendMail');
const look = require('./look.js');
const saveAndSend = require('./saveAndSend.js');
...
function getAllWatchesForUser(userId) {
  const allWatches = model.getAllWatches().filter(watch => watch.userId === userId)
    .map(watch => {
      return {
        url: watch.url,
...
async function watch(watches) {
  const objs = await bodySnatcher.getPageBodies(watches);
  const bodies = objs.map(obj => obj.body);
  watches.forEach((watch, i) => {
    const hostname = (new URL(watch.url)).hostname;
    console.log(`Looking at ${hostname} for ${watch.watchName}`);
    _watchSite(watch, bodies[i]);
  });
}
...
async function _watchSite(options, body) {
  try {
    cjl(options);
    const lookResults = await look(options, body);
    console.log(`Got listings from -> ${lookResults.url}`);
    const saveAndSendResults = await saveAndSend(options, lookResults.listings);
  } catch (e) {
    console.error(e);
  }
}
