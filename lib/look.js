...
const rssHtmlSelectors = require('./rssHtmlSelectors.json')
const { JSDOM } = jsdom;
const fs = require('fs')
...
function look (options, bodyString) {
  ...
  const promise = new Promise(async function (resolve, reject) {
    ...
    const cleanedBodyString = cleanBody(bodyString, options, hostname)
    // fs.writeFileSync('./test.html', cleanedBodyString)
    const { document } = (new JSDOM(cleanedBodyString)).window;
    const listings = [];
    const quals = options.selectors || rssHtmlSelectors;
    const listingEls = Array.from(document.querySelectorAll(quals.listing))
      .filter(el => quals.ignore ? !el.querySelector(quals.ignore) : true);
    const listingLength = listingEls.length; // to know when to resolve
