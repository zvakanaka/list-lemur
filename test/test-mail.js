...
describe('Building body of email', function () {
  it('should specify if multiple listings are passed in', function (done) {
    const subject = sendMail._buildSubject(listings);
    const body = sendMail._buildBody(listings, subject);
  ...
  it('should specify if one listing is passed in', function (done) {
    const subject = sendMail._buildSubject([listings[0]]);
    const body = sendMail._buildBody([listings[0]], subject);
...
if (sendTo !== undefined) {
  describe('Sending test SMS', function () {
    it('should be received', function (done) {
      ...
      var prefix = 'You have 15 seconds. Did you receive a text?[y,n] ';
